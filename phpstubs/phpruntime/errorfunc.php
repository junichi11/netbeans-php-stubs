 ...
 * @param callable $error_handler <p>次のシグネチャに従うコールバック。 <b><code>NULL</code></b> を渡すと、ハンドラをデフォルトの状態に戻せます。 関数名のかわりにオブジェクトへのリファレンスとメソッド名を含む配列を指定することもできます。</p> <p></p> bool handler ( int <code>$errno</code> , string <code>$errstr</code> [, string <code>$errfile</code> [, int <code>$errline</code> [, array <code>$errcontext</code> ]]] )   <code>errno</code>    最初のパラメータ <code>errno</code> は、発生させる エラーのレベルを整数で格納します。    <code>errstr</code>    2 番目のパラメータ <code>errstr</code> は、 エラーメッセージを文字列で格納します。    <code>errfile</code>    3 番目のパラメータ <code>errfile</code> はオプションで、 エラーが発生したファイルの名前を文字列で格納します。    <code>errline</code>    4 番目のパラメータ <code>errline</code> はオプションで、 エラーが発生した行番号を整数で格納します。    <code>errcontext</code>    5 番目のパラメータ <code>errcontext</code> はオプションで、 エラーが発生した場所のアクティブシンボルテーブルを指す配列です。 つまり、エラーが発生したスコープ内でのすべての変数の内容を格納した 配列が <code>errcontext</code> だということです。 ユーザーエラーハンドラは、決してエラーコンテキストを書き換えては いけません。   <b>警告</b> <p>This parameter has been <i>DEPRECATED</i> as of PHP 7.2.0. Relying on it is highly discouraged.</p>    <p>この関数が <b><code>FALSE</code></b> を返した場合は、通常のエラーハンドラが処理を引き継ぎます。</p>
 ...
 * @param callable $exception_handler <p>キャッチされない例外が発生した際にコールされる関数の名前。 このハンドラ関数は、パラメータをひとつとる必要があります。 このパラメータは、スローされた例外オブジェクトとなります。 PHP 7 より前のバージョンでは、ハンドラのシグネチャは、次のようになります。</p> <p></p> void handler ( <code>Exception</code> <code>$ex</code> ) <p>PHP 7 からは、大半のエラーが Error 例外として報告されるようになりました。 これらもこのハンドラで扱うことになるでしょう。Error と Exception は、どちらも Throwable インターフェイスを実装しています。 PHP 7 以降のハンドラのシグネチャは、次のようになります。</p> <p></p> void handler ( <code>Throwable</code> <code>$ex</code> ) <p><b><code>NULL</code></b> を渡すと、このハンドラをデフォルトの状態に戻せます。</p>  <b>警告</b> <p>コールバックの <code>ex</code> パラメータにタイプヒントとして Exception を明示すると、 PHP 7 での例外の階層の変更に伴う問題が発生することに注意しましょう。</p>
