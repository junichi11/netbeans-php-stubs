 ...
 * @return string <p>ハッシュしたパスワードを返します。失敗した場合に <b><code>FALSE</code></b> を返します。</p><p>使ったアルゴリズムやコスト、そしてソフトもハッシュの一部として返されます。 つまり、ハッシュを検証するために必要な情報は、すべてそこに含まれているということです。 そのため、<code>password_verify()</code> でハッシュを検証するときに、 ソルトやアルゴリズムの情報を別に保存する必要はありません。</p>
 ...
 * <p>指定したハッシュが、特定のアルゴリズムとオプションを満たしているかどうかを調べます。 満たしていない場合は、ハッシュを再計算する必要があるとみなします。 rehashed.</p>
 * @param string $hash <p><code>password_hash()</code> が作ったハッシュ。</p>
 * @param int $algo <p>パスワードのハッシュに使うアルゴリズムを表す パスワードアルゴリズム定数。</p>
 * @param array $options <p>オプションを含む連想配列。各アルゴリズムがサポートするオプションについては、 パスワードアルゴリズム定数 のページを参照ください。</p>
 * @return boolean <p>指定した <code>algo</code> と <code>options</code> にマッチするためにハッシュの再計算が必要な場合は <b><code>TRUE</code></b>、 それ以外の場合は <b><code>FALSE</code></b> を返します。</p>
 ...
 * @param string $hash <p><code>password_hash()</code> が作ったハッシュ。</p>
 * @return boolean <p>パスワードとハッシュがマッチする場合に <b><code>TRUE</code></b>、それ以外の場合に <b><code>FALSE</code></b> を返します。</p>
