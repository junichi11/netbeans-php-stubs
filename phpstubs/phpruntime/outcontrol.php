 ...
 * <p>現在のバッファの中身を取得し、出力バッファを削除します。</p><p><b>ob_get_clean()</b> は、基本的に <code>ob_get_contents()</code> および <code>ob_end_clean()</code> を同時に実行するのと同じです。</p><p>出力バッファを開始するときに、 <code>ob_start()</code> で PHP_OUTPUT_HANDLER_CLEANABLE フラグを指定する必要があります。指定しなければ、<b>ob_get_clean()</b> は動作しません。</p>
