...
namespace {
	 ...
	 * @link https://php.net/manual/ja/function.preg-filter.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-grep.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-last-error.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-match.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-match-all.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-quote.php
	 * @see escapeshellcmd()
	 * @since PHP 4, PHP 5, PHP 7
	 */
	 ...
	 * @link https://php.net/manual/ja/function.preg-replace.php
	 ...
	 * @param callable $callback <p>このコールバック関数は、検索対象文字列でマッチした要素の配列が指定されて コールされます。このコールバック関数は、置換後の文字列を返す必要があります。 コールバックのシグネチャは、次のとおりです。</p> <p></p> handler ( array <code>$matches</code> ) : string  <p><b>preg_replace_callback()</b> 用の <code>callback</code> 関数が、 ひとつの場所だけで必要となることがあります。 そんな場合は、 無名関数 を使って宣言した無名関数を <b>preg_replace_callback()</b> のコール時に使用します。 このようにすることにより、コールに関するすべての情報を 1 ヶ所に集め、 他の場所で使用されないコールバック関数名で関数の名前空間を 汚染しないようにすることができます。</p> <p></p> <p><b>例1 <b>preg_replace_callback()</b> と無名関数 </b></p>  <code> &lt;&#63;php<br>/&#42; Unix 方式のコマンドラインフィルタです。<br> &#42; 段落の冒頭の大文字を、小文字に変換します。&#42;/<br>$fp = fopen("php://stdin", "r") or die("標準入力から読み込めません");<br>while (!feof($fp)) {<br>    $line = fgets($fp);<br>    $line = preg_replace_callback(<br>        '|&lt;p&gt;\s&#42;\w|',<br>        function ($matches) {<br>            return strtolower($matches[0]);<br>        },<br>        $line<br>    );<br>    echo $line;<br>}<br>fclose($fp);<br>&#63;&gt;  </code>
	 ...
	 * @link https://php.net/manual/ja/function.preg-replace-callback.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-replace-callback-array.php
	 ...
	 * @link https://php.net/manual/ja/function.preg-split.php
	...
	/**
	 * <b><code>PREG_SPLIT_OFFSET_CAPTURE</code></b> の説明を参照してください。
	 */
	...
	/**
	 * このフラグは、<code>preg_split()</code> が 文字列分割用のパターン中のカッコによるサブパターンでキャプチャされた値も 同時に返すようにします。
	 */
	...
	/**
	 * このフラグを設定した場合、各マッチに対応する文字列のオフセットも返されます。 これにより、返り値は配列となり、配列の要素 0 はマッチした文字列、 要素 1 は対象文字列中におけるマッチした文字列のオフセット値となることに注意してください。 このフラグは <code>preg_split()</code> のみで使用されます。
	 */
